...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "rounded-xl border border-white/5 bg-card/60 text-card-foreground shadow-lg backdrop-blur-xl transition-all duration-300 hover:shadow-xl hover:border-white/10",
        className
      )}
      {...props}
    />
